 ...
 *key. Iterator status unchanged mapRemove		- Removes a pair of
 *(key,data) elements for which the key matches a given element (using the
 *strcmp function). mapGetFirst	- Sets the internal iterator to the first key in
 *the map, and returns it. mapGetNext		- Advances the internal iterator
 *to the next key and returns it. mapClear		- Clears the contents of
 *the map. Frees all the elements of the map using the free function.
 ...
 * equal using the strcmp function from string.h.
 *
 * @param map - The map to search in
 * @param key - The key to look for.
 * @return
 * 	false - if one or more of the inputs is null, or if the key element was
 * not found. true - if the key element was found in the map.
 */
 ...
 * @param key - The key element which need to be assigned/reassigned.
 *       A copy of the key element will be inserted.
 * @param data - The new data element to associate with the given key.
 *      A copy of the data element will be inserted and old data memory would be
 *deleted.
 * @return
 * 	MAP_NULL_ARGUMENT if one of the params is NULL
 * 	MAP_OUT_OF_MEMORY if an allocation failed (Meaning the function for
 *copying an element failed) MAP_SUCCESS the paired elements had been inserted
 *successfully
 */
MapResult mapPut(Map map, const char* key, const char* data);

/**
*	mapGet: Returns the data associated with a specific key in the map(not
a copy). *			Iterator status unchanged
*
* @param map - The map for which to get the data element from.
* @param key - The key element which need to be found and who's data
we want to get.
* @return
 ...
 * elements are found using the comparison function strcmp. Once found, the
 * elements are removed and deallocated. Iterator's value is undefined after
 * this operation.
 *
 * @param map -
 * 	The map to remove the elements from.
 * @param key
 * 	The key element to find and remove from the map. The key element will be
 * freed. The data element associated with this key will also be freed.
 * @return
 ...
 * map MAP_SUCCESS the paired elements had been removed successfully
 */
MapResult mapRemove(Map map, const char* key);

/**
 *	mapGetFirst: Sets the internal iterator (also called current key
 *element) to the first key element in the map. There doesn't need to be an
 *internal order of the keys so the "first" key element is any key element. Use
 *this to start iterating over the map. To continue iteration use mapGetNext
 *
...
/**
 *	mapGetNext: Advances the map iterator to the next key element and
 *returns it. The next key element is any key element not previously returned by
 *the iterator.
 * @param map - The map for which to advance the iterator
 * @return
 * 	NULL if reached the end of the map, or the iterator is at an invalid
 *state or a NULL sent as argument The next key element on the map in case of
 *success
 */
...
#define MAP_FOREACH(iterator, map)                           \
    for (char* iterator = (char*)mapGetFirst(map); iterator; \
